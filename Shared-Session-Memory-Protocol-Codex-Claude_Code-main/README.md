...
This prompt directly references the `Spec ID` and `operating mode`, creating an unbroken chain of development where the user simply copies the instruction prepared by the previous session based on the code state.
...
This protocol transforms AI development from “informal chatting with a bot” into a strictly disciplined engineering process where every step is traceable, provable, and automated through the repository’s structure.
